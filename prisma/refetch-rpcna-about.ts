// Re-fetch about-pages for RPCNA churches whose websites were just corrected
...
async function worker(queue: Church[], results: any[], counter: { n: number; total: number }) {
  while (queue.length) {
    ...
    results.push(r)
    counter.n++
    console.log(`  ${counter.n}/${counter.total} #${c.id} ${r.status} (${r.pages.length} pages)`)
  }
}

async function main() {
  // Load full list, filter to RPCNA with website (the corrected ones)
  const list: Church[] = JSON.parse(fs.readFileSync('data/research-queue/enriched_193.json', 'utf-8'))
  const targets = list.filter(c => c.denom === 'RPCNA' && c.hasWebsite && c.website)
  console.log(`Re-fetching ${targets.length} RPCNA churches with corrected websites (concurrency 3)...\n`)

  const queue = [...targets]
  const results: any[] = []
  const counter = { n: 0, total: targets.length }
  const start = Date.now()
  await Promise.all(Array.from({ length: 3 }, () => worker(queue, results, counter)))
  const elapsed = ((Date.now() - start) / 1000).toFixed(1)

  fs.writeFileSync('data/research-queue/refetch_rpcna_manifest.json', JSON.stringify(results, null, 2))
