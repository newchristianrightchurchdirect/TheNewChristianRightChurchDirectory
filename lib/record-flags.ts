...
export const RECORD_FLAGS: FlagDef[] = [
  ...
  { key: 'city_is_county', label: 'City field holds a county', held: false,
    description: 'A bulk import put the COUNTY in the city field ("Stark County"), and no zip or street address was available to recover the real city. The location is roughly right but not a city, and any code that matches on city will silently miss this row — which is how several duplicates hid. Fix the city before trusting a city-based match.' },
  { key: 'location_conflict', label: 'Location fields disagree', held: false,
    description: 'The state, zip and city on this row do not agree with one another — e.g. an Alabama row carrying a Pennsylvania zip. One of them is wrong and it has not been established which.' },
